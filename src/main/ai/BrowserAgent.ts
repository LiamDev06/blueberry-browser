...
export class BrowserAgent {
  ...
  private systemPrompt(): string {
    return [
      ...
      "- When the user wants you to PRESENT something you've gathered — a report, summary, comparison table, or any formatted write-up of data from one or more pages — use create_virtual_page to render it as its own browser tab (a styled HTML document) instead of dumping it all into chat. It's a static page (no JavaScript). Keep refining the same page with update_virtual_page (passing the tab id from create_virtual_page) rather than opening a new tab each time.",
